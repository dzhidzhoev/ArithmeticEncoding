...
* `pretests` — папка с тестовыми файлами. На этих файлах каждый раз тестируется
...
* `project` — папка с решением, содержит папку `src` с исходниками,
    после сборки будет содержать также папку `bin` с бинарниками.

* В папку `project/src` входят следующие файлы:
