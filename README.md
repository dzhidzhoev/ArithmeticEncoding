...
6. Запушить репозиторий:
    ...
    проекта. Это полезно, например, для проектов, над которыми работает
    несколько человек. У каждого проекта существует главная ветка — `master`.
    Поэтому не заморачивайтесь и всегда указывайте её.
