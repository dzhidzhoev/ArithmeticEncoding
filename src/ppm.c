...
enum { FREQS_TABLE_DEPTH = 2 };

typedef struct _freqs_table_entry {
    unsigned freq;
    int sym;
    struct _freqs_table_entry *subtable;
} freqs_table_entry;

freqs_table_entry *create_freqs_table() {
    freqs_table_entry *res = malloc(sizeof(*res) * N);
    for (int i = 0; i < N; i++) {
        res[i].freq = 1;
        res[i].sym = i;
        res[i].subtable = NULL;
...
static freqs_table_entry *global_table;
static freqs_table_entry *freqs;
...
static const unsigned long long FREQ_ADD_COEFF = (1 << 8);
static const int FREQS_SCALE_COEFF = 6;

static int sym_history[FREQS_TABLE_DEPTH];
...
static void init_tables() {
    ...
    global_table = create_freqs_table();
    freqs_summ = N;
    freqs = global_table;

    for (int i = 0; i < FREQS_TABLE_DEPTH; i++) {
        sym_history[i] = 0;
    }
}
...
static void modify_table(int ch) {
    ...
    global_table[ch].freq += FREQ_ADD_COEFF;
    counts[ch]++;
    ++read_bytes;

    for (int i = 1; i < FREQS_TABLE_DEPTH; i++) {
        sym_history[i - 1] = sym_history[i];
    }
    sym_history[FREQS_TABLE_DEPTH - 1] = ch;
    freqs_table_entry *new_freqs = global_table;
    for (int i = 0; i < FREQS_TABLE_DEPTH; i++) {
        if (!new_freqs[sym_history[i]].subtable) {
            new_freqs[sym_history[i]].subtable = create_freqs_table();
        }
        new_freqs = new_freqs[sym_history[i]].subtable;
    }
    freqs = new_freqs;
}
